...
int main()
{
    ...
    for (int i = 1; i <= 4; ++i)
    {
        ...
        {
            cout << "Multiple servers with infinite queue" << endl;
        ...
        {
            cout << "Multiple servers with finite queue" << endl;
            cout << "Enter the number of security scanners in the system: ";
            cin >> num_of_servers;
            cout << "Enter the size of buffer: ";
            cin >> size_of_buffer;
        ...
        vector<double> completion_time(num_of_servers, -1);
        ...
        while (index < arrival_times.size())
        {
            for (int i = 0; i < num_of_servers; ++i)
            {
                while (completion_time[i] != -1 && completion_time[i] <= arrival_times[index])
                {
                    if (occupied[i].empty())
                    {
                        cout << "Time " << completion_time[i] << ": Server " << i 
                             << " has completed servicing a passenger." << endl;
                    ...
                    {
                        cout << "Time " << completion_time[i] << ": Server " << i 
                             << " has completed servicing a passenger and now has started servicing a passenger who had"
                             << " arrived at time = " << occupied[i].front() 
                             << " and the passenger's servicing will be completed at time = " 
                             << completion_time[i] << "." << endl;
            ...
            if (!free_servers.empty())
            {
                ...
                cout << "Time " << arrival_times[index] << ": Server " << free_servers[x] 
                     << " has started servicing the passenger who just arrived. Servicing will be completed at time "
                     << completion_time[free_servers[x]] << "." << endl;
            }
            else
            {
                int ind = -1;
                ...
                else
                {
                    ...
                    cout << "Time " << arrival_times[index] << ": The passenger has joined the queue of server " << ind << "." << endl;
                }
            }
        ...
        cout << endl << "Arrivals done!!!" << endl << endl;
        ...
        for (int i = 0; i < num_of_servers; ++i)
        {
            while (completion_time[i] != -1)
            {
                ...
                else
                {
                    ...
                    occupied[i].pop();
                    cout << "Time " << completion_time[i] << ": Server " << i 
                         << " has completed servicing a passenger and now has started servicing a passenger who had arrived "
                         << "at time = " << x << " and the servicing of the passenger will be completed at " 
                         << completion_time[i] << "." << endl;
        ...
        double avg_wait_time = (num_waiting > 0) ? (total_wait_time / num_waiting) : 0;
